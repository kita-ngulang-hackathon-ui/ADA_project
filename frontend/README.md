...
`NEXT_PUBLIC_API_BASE_URL` points at the deployed backend (`../backend`), which
is hosted separately. That backend must list this app's origin in its
`API_CORS_ORIGINS`, or the browser blocks every console call.
